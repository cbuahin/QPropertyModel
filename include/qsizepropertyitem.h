/*!
 * \file qsizepropertyitem.h
 * \author Caleb Buahin <caleb.buahin@gmail.com>
 * \version 1.0.0
 * \description
 * Property item for QSize type with expandable Width/Height children.
 * \license
...
/*!
 * \brief Property item for QSize with expandable Width and Height children.
 *
 * \details The value column displays the size as \c "w x h".  Expanding the
 * item reveals two integer spin-box children labelled \b Width and \b Height.
 * Changes to either child are aggregated and written back to the underlying
 * QMetaProperty.
 */
class QSizePropertyItem : public QVariantPropertyItem
{
   ...
   public:
      /*!
       * \brief Constructs a size property item.
       *
       * \param[in] value        The initial QSize value wrapped in a QVariant.
       ...
       *
       * \details For column 1, Qt::DisplayRole returns \c "w x h".
       * Qt::EditRole returns the raw QSize wrapped in a QVariant.
       *
   ...
   private slots:
       ...
       *
       * \details Re-assembles the QSize from the current child values and writes
       * the updated size to the underlying QMetaProperty.
       *
       * \param[in] name  The child property name (\c "Width" or \c "Height").
       * \param[in] value The new integer dimension value.
       */
